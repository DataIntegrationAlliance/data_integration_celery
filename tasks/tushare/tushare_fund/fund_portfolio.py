...
def invoke_fund_portfolio(ts_code):
    invoke_fund_portfolio = pro.fund_portfolio(ts_code=ts_code)
    return invoke_fund_portfolio

INDICATOR_PARAM_LIST_TUSHARE_FUND_PORTFOLIO = [
    ('ts_code', String(20)),
    ('ann_date', Date),
    ('end_date', Date),
    ('symbol', String(20)),
...
# 设置 dtype
DTYPE_TUSHARE_FUND_PORTFOLIO = {key: val for key, val in INDICATOR_PARAM_LIST_TUSHARE_FUND_PORTFOLIO}
    ...
    try:
        for num, (ts_code) in enumerate(code_list, start=1):
            ...
            if data_count >= 20000:
                ...
                bunch_insert_on_duplicate_update(data_df_all, table_name, engine_md, DTYPE_TUSHARE_FUND_PORTFOLIO)
        ...
        if len(data_df_list) > 0:
            data_df_all = pd.concat(data_df_list)
            data_count = bunch_insert_on_duplicate_update(data_df_all, table_name, engine_md, DTYPE_TUSHARE_FUND_PORTFOLIO)
            all_data_count = all_data_count + data_count
            logging.info("更新 %s 结束 %d 条信息被更新", table_name, all_data_count)

if __name__ == "__main__":
    # DEBUG = True
    import_tushare_fund_portfolio()