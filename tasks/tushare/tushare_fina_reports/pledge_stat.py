...
def import_tushare_stock_pledge_stat(chain_param=None,ts_code_set=None):
    ...
    try:
        for ts_code in ts_code_list:
            ...
            # 大于阀值有开始插入
            if data_count >= 10000 and len(data_df_list)>0 :
                data_df_all = pd.concat(data_df_list)
                data_count=bunch_insert_on_duplicate_update(data_df_all, table_name, engine_md, DTYPE_TUSHARE_STOCK_PLEDGE_STAT)
