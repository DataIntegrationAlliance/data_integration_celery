...
def import_tushare_stock_fina_mainbz(chain_param=None,ts_code_set=None):
    ...
    try:
            ...
            for mainbz_type in list(['P', 'D']):
                ...
                if len(data_df) > 0 and data_df['ann_date'].iloc[-1] is not None:
                    while try_2_date(data_df['end_date'].iloc[-1]) > date_from:
                        ...
                        if len(df2) > 0 and df2['ann_date'].iloc[-1] is None:
                ...
                if data_df is None:
                    ...
                    logger.info('整体进度：%d/%d)， %d 条  %s 的 %s类型统计之主营业务数据被提取，起止时间为 %s 和 %s', num, data_len,
                                data_df.shape[0], ts_code, mainbz_type, date_from, date_to)
                ...
                if data_count >= 500 and len(data_df_list) > 0:
                    ...
                    bunch_insert_on_duplicate_update(data_df_all, table_name, engine_md,DTYPE_TUSHARE_STOCK_FINA_MAINBZ)
        ...
        if len(data_df_list) > 0:
            data_df_all = pd.concat(data_df_list)
            data_count = bunch_insert_on_duplicate_update(data_df_all, table_name, engine_md, DTYPE_TUSHARE_STOCK_FINA_MAINBZ)
            all_data_count = all_data_count + data_count
            logging.info("更新 %s 结束 %d 条主营业务信息被更新", table_name, all_data_count)
